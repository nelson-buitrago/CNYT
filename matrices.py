...
def suma_Vector(a,b):
    ...
    if len(a)!=len(b):
        ...
        for i in range(fila):
            rtaVector[i][0] = laboratorio1.suma(a[i][0],b[i][0])
...
def resta_Vector(a,b):
    ...
    if len(a)!=len(b):
        ...
        for i in range(fila):
            rtaVector[i][0] = laboratorio1.resta(a[i][0],b[i][0])
...
def inversoAditivo_Vector(a):
    ...
    for i in range(fila):
        rtaVector[i][0] = laboratorio1.producto(a[i][0],[-1,0])
    ...
    if len(a)!=len(b) or len(a[0])!=len(b[0]):
        ...
        for i in range(fila):
            for j in range(columna):
                rtaMatriz[i][j] = laboratorio1.suma(a[i][j],b[i][j])
...
def productoTensorial_Matriz_Vector(a,b):
    ...
    for i in range(len(nuevaMatriz)):
        for j in range(len(nuevaMatriz[0])):
            nuevaMatriz[i][j] = laboratorio1.producto(a[i//filaM2][j//columnaM2],b[i%filaM2][j%columnaM2])
